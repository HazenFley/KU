({
    ...
    handleSaveInvention: function(component, event, helper) {
        if(helper.validateInventionForm(component)) {
            ...
            //Location is selected from the picklist, so we set it in the field set
            console.log("Location__c = " + component.find('invLocation').get("v.value"));
            component.set("v.simpleInvention.Location__c", component.find('invLocation').get("v.value"));
